...
- Add tab-local presentation receipts using the PR-panel offered-marker pattern.
  Include user/workspace/task/canvas identity and a memory fallback for storage errors.
...
```bash
(cd apps/web && pnpm exec vitest run components/task/dockview-canvas-activation.test.tsx hooks/domains/task/use-task-canvases.test.ts lib/canvas-presentation-storage.test.ts components/settings/canvas-host-route.test.tsx components/task/dockview-add-panel-items.test.tsx)
...
an offer. New tabs intentionally have independent presentation history.
...
previous task's settled layout.

Recorded verification:

- Focused Vitest suite passed: 5 files, 56 tests, including deferred
  reconnect/lifecycle invalidation, authenticated identity, mixed inventory,
  delayed restoration, ownership recheck, batch focus, and receipt cases.
