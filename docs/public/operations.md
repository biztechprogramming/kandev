...
- **Unread divider**: a per-user setting at **Settings > Preferences > Task Behavior > Conversation and panels**. It defaults off, takes effect immediately, and controls both the Slack-style **New** divider and read-cursor updates while that user's transcript view is visible.
